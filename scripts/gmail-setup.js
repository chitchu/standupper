 ...
 *   3. Run: npm run gmail-setup
 *   4. Open the printed URL in your browser and approve access
 *   5. Copy the printed GMAIL_REFRESH_TOKEN= line into your .env
 */
...
if (existsSync(secretPath)) {
  ...
  // For installed apps Google allows any localhost port; use our local server
  redirectUri = `http://localhost:${PORT}/callback`
  console.log(`Loaded credentials from client-secret.json (type: ${json.web ? 'web' : 'installed'})`)
} else {
  const { GMAIL_CLIENT_ID, GMAIL_CLIENT_SECRET } = process.env
  if (!GMAIL_CLIENT_ID || !GMAIL_CLIENT_SECRET) {
    console.error('Error: place client-secret.json in the project root, or set GMAIL_CLIENT_ID and GMAIL_CLIENT_SECRET in .env')
    process.exit(1)
  }
